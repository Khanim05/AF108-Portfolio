// 1.Gosterdiyim kimi heart iconlari sehifeden cixdiqdan sonra seclimis olaraq qalmalidir.
// 2.Login olmadiqda wishlist sehifesi acilmamalidir login sehifesine kecid olmalidir.(Toast olaraq) +hazir
// 3.Productlari secdikden sonra derhal logout olduqda productlar secilmis olaraq qalmamalidir. +hzir
// 4.Logout olduqda toast olaraq xeberdarliq olmalidir. +hazir
...
document.addEventListener("DOMContentLoaded", async()=>{
  ...
  function createUserCard() {
     products.forEach(product => {
        ...
        card.append(cardHeart,cardImage,cardContent)
  ...
  function Adduserwishlist(productId,cardHeart) {
    // console.log(productId)
    if(!loginedUser){
    toast("Sevimlilərə məhsul əlavə etmək üçün login ol.")
    setTimeout(() => {
    ...
    let loginedUserProduct=loginedUser.wishlist.some((item)=>item.id===productId)
  ...
  function AdduserBasket(productId) {

    if(!loginedUser){
      toast("Səbətə məhsul əlavə etmək üçün login ol.")
      setTimeout(() => {
          window.location.href="login.html"
      }, 1000);
      }

    let existproducts=basket.find((elem)=>elem.id===productId)
